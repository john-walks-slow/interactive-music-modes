...
const DiatonicChords: React.FC<DiatonicChordsProps> = ({ mode, tonic, onNotesAnimate, animatingChordIndices, onChordPlay }) => {
  const [chordType, setChordType] = useState<ChordType>('triad');

  const chords = getDiatonicChords(mode, tonic, chordType);

  const baseClasses = 'px-4 py-2 text-sm font-medium transition-colors duration-200 focus:outline-none focus:z-10 focus:ring-2 focus:ring-sky-500';
      ...
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
        ...
        <div className="inline-flex rounded-md shadow-sm self-start sm:self-center" role="group">
          <button
            ...
            className={`${baseClasses} rounded-l-lg border border-gray-200 ${chordType === 'triad' ? activeClasses : inactiveClasses}`}
          >
            Triads
          </button>
          <button
            ...
            className={`${baseClasses} rounded-r-md border-t border-b border-r border-gray-200 ${chordType === 'seventh' ? activeClasses : inactiveClasses}`}
          >
            Seventh Chords
          </button>
        </div>
