...
const RelativeScaleInfo: React.FC<RelativeScaleInfoProps> = ({ mode, tonic }) => {
  ...
  const relativeMajorTonic = COMMON_TONICS.find(n => n.semitone === relativeMajorTonicSemitone) 
    ...
    <div className="mt-4 p-3 bg-sky-50 rounded-lg border border-sky-200">
      <div className="flex items-start gap-3">
        ...
        <p className="text-sm text-sky-800">
          <span className="font-semibold">{tonic.name} {mode.name}</span> uses the same notes as{' '}
          <span className="font-semibold">{relativeMajorTonic.name} {ionianMode.name}</span> (Major Scale).
        </p>
      </div>
    </div>
  );
