 ...
 */
const getIntervalStyle = (interval: Interval | undefined, mode: Mode, onDark: boolean): string => {
    // if (!interval) return onDark ? 'text-gray-400' : 'text-gray-500';
    return onDark ? 'text-gray-300' : 'text-gray-600';
    const isCharacteristic = mode.characteristicIntervals?.includes(interval.number) && mode.derivation?.parent !== mode.name;
    ...
    if (interval.number === 3) {
        if (interval.quality === 'M') return onDark ? 'text-sky-200 font-bold' : 'text-sky-600 font-bold';
        if (interval.quality === 'm') return onDark ? 'text-yellow-200 font-bold' : 'text-yellow-600 font-bold';
    }
    // Standard colors for other interval qualities.
    switch (interval.quality) {
        case 'M': return onDark ? 'text-sky-300' : 'text-sky-500';
        case 'm': return onDark ? 'text-yellow-300' : 'text-yellow-500';
        case 'A': return onDark ? 'text-green-300' : 'text-green-600';
        case 'd': return onDark ? 'text-red-300' : 'text-red-600';
        case 'P':
        default: return onDark ? 'text-gray-300' : 'text-gray-600';
    }
};


const Piano: React.FC<PianoProps> = ({ mode, tonic, scaleNotes, animationTriggers, onNoteClick }) => {
  ...
  return (
    <div className="relative w-full h-full select-none overflow-hidden rounded-lg border-2 border-gray-200 bg-slate-100 shadow-inner">
      ...
      {whiteKeys.map((note, index) => {
        ...
        const label = isScaleNote ? semitoneToIntervalMap.get(relativeSemitone) : '';
        const intervalObject = isScaleNote ? semitoneToIntervalObjectMap.get(relativeSemitone) : undefined;
        ...
        return (
          ...
          >
            {label && (
              <span className={`absolute bottom-2 left-1/2 -translate-x-1/2 text-sm font-mono ${getIntervalStyle(intervalObject, mode, false)}`}>
      ...
      {keysToRender.map((note, keyIndex) => {
        ...
        const whiteKeysBefore = keysToRender.slice(0, keyIndex).filter(n => !n.isBlack).length;
        const leftPosition = whiteKeysBefore * WHITE_KEY_WIDTH_PERCENT - (BLACK_KEY_WIDTH_PERCENT / 2);
        ...
        return (
          ...
          >
            {label && (
                <span className={`absolute bottom-2 left-1/2 -translate-x-1/2 text-sm font-mono ${getIntervalStyle(intervalObject, mode, true)}`}>
                    {label}
                </span>
            )}
          </div>
