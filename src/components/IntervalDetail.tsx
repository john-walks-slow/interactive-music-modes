...
const AlterationIndicator: React.FC<{ change: 'raised' | 'lowered' }> = ({
  change,
}) => {
...
const IntervalDetail: React.FC<IntervalDetailProps> = ({ mode }) => {
  ...
  const parentModeKey =
    comparisonTarget === 'auto' ? mode.derivation.parent : comparisonTarget
  ...
  return (
    <div>
      <div className="flex items-baseline mb-4">
        <h3 className="text-base font-semibold text-gray-800">Compared with</h3>
      ...
      <div className="flex flex-wrap gap-2">
        {mode.intervals.map((interval, index) => {
          ...
          const baseClasses =
            'px-3 py-1.5 rounded-md text-sm font-mono flex items-center justify-center transition-colors duration-200'
