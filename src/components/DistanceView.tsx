...
const DegreeNode: React.FC<{
  ...
  return (
    ...
    >
      <div
        className={`w-full h-full rounded-full bg-sky-200 flex items-center justify-center font-bold text-sky-800 text-[9px] md:text-base border border-sky-300 md:border-2 transition-transform ${onClick ? 'active:scale-95' : ''}`}
      >
        {label}
      </div>
  ...
  const getFlexGrow = () => {
    switch (step) {
      ...
      case 'H':
        return 1
      case 'WH':
        return 3 // For Harmonic Minor's augmented 2nd
      default:
        return 1
    }
  }
...
const DistanceView: React.FC<DistanceViewProps> = ({
  mode,
  tonic,
  scaleNotes,
  animationTriggers,
