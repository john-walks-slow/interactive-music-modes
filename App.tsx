...
import { getFrequencyFromSemitone, getScaleNotesWithAbsoluteSemitones } from './utils/music';
...
const App: React.FC = () => {
  ...
  const [animationTriggers, setAnimationTriggers] = useState(new Map<number, string>());
  const [animatingChordIndices, setAnimatingChordIndices] = useState(new Map<number, string>());
  ...
  const onNotesAnimate = useCallback((notes: Note[]) => {
    ...
    setAnimationTriggers(prev => {
        ...
        semitonesToAnimate.forEach(semitone => {
            ...
            next.set(semitone, currentName === 'note-pop-a' ? 'note-pop-b' : 'note-pop-a');
        });
        return next;
    });
  ...
  const handleChordPlay = useCallback((degree: number) => {
    ...
    setAnimatingChordIndices(prev => {
      ...
      next.set(degree, currentName === 'note-pop-a' ? 'note-pop-b' : 'note-pop-a');
      return next;
    });
